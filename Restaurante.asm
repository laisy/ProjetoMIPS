...
#----------------------------------Menu Principal---------------------------------------------------------------------------------------------------
	...
	addi $a1, $zero, 4	#Parâmetro pra saber se a opção escolhida é menor ou igual a 4
	jal verificacao		#Função que verifica se a opção escolhida é um número entre 1 e 4 [ verificacao(0, 4) ]
...
subMenu:	beq $a0, 1, menuCliente		#Menu do cliente foi escolhido
		...
		beq $a0, 4, menuPedidos		#Menu dos pedidos foi escolhido
#xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
...
menuCliente: 	la $a0, opcaoCliente	#Carrega o menu do cliente
		...
		addi $a1, $zero, 4	#Parâmetro pra saber se a opção escolhida é menor ou igual a 4
		jal verificacao		#Função que verifica se a opção escolhida é um número entre 1 e 4 [ verificacao(0, 4) ]
		jal acaoCliente		#Chamando a função de verificação de escolha
...
menuFuncionario: 	la $a0, opcaoFuncionario	#Carrega o menu do funcionario
			...
			addi $a1, $zero, 4		#Parâmetro pra saber se a opção escolhida é menor ou igual a 4
			jal verificacao			#Função que verifica se a opção escolhida é um número entre 1 e 4 [ verificacao(0, 4) ]
			jal acaoFuncionario		#Chamando a função de verificação de escolha
...
#===========================================================FUNCIONARIO===============================================================================
#-------------------------------------------------ESCOLHA----------------------------------------------------------------------------------
acaoFuncionario:	beq $a2, 1, funcaoCadastrarFunc		#Chamada da função de cadastro de funcionario escolhida
			beq $a2, 2, funcaoRemoverFunc		#Chamada da funcao de remoção de funcionario escolhida
			beq $a2, 3, funcaoEditarFunc		#Chamada da função de edição de funcionario escolhida
...
rebobinarRemocaoFunc:	add $t0, $zero, $a2	#Armazenando a quantidade de caracteres excluidos
			lw $a2, 0($sp)		#Pegando o valor original do indice
			addi $sp, $sp, 4	#Desalocando o espaço usado na pilha
			add $a2, $a2, $t0	#Somando o indice, para saber o quanto já foi lido
			add $a0, $zero, $s1	#Garantindo que, caso já tenha acabado o arquivo de leitura, o arquivo de escrita vai ser fechado primeiro.
...
removerObjetoFunc:	beq $a2, 64, rebobinarRemocaoFunc		#Se já tiver ignorado o objeto completamente, volta a armazenar o resto dos caracteres
			add $t1, $zero, $a2				#Salvando temporariamente a quantidade de caracteres até a posição do objeto
			la $a1, byte					#Informando onde os caracteres (ignorados) vão ficar
			addi $a2, $zero, 1				#Informando que só vai ler 1 caracter
			jal lerDoArquivo				#Lendo o caracter
